...
Statistic::Statistic(std::byte const* data, model::TypeId id, bool clone_data /* = false */) {
    if (data != nullptr) {
        switch (id) {
            case +mo::TypeId::kInt:
                this->type_ = std::make_unique<mo::IntType>(mo::IntType());
                if (clone_data)
            ...
            case +mo::TypeId::kDouble:
                this->type_ = std::make_unique<mo::DoubleType>(mo::DoubleType());
                if (clone_data)
            ...
            case +mo::TypeId::kString:
                this->type_ = std::make_unique<mo::StringType>(mo::StringType());
                if (clone_data)
                    data_ = mo::DoubleType().Clone(data);
                else
                    data_ = data;
